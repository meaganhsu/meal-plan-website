...
const Calendar = () => {
    ...
    // calculating min and max dates for navigation
    ...
    const handleSwapComplete = async (targetDay, targetMeal) => {
        ...
        if (!sourcePlan) {
            sourcePlan = await fetchMealPlanForWeek(getWeekStart(swapSource.date));
        }
        if (!targetPlan && getWeekStart(targetDay) !== getWeekStart(swapSource.date)) {
            targetPlan = await fetchMealPlanForWeek(getWeekStart(targetDay));
        }

        // get dishes from both slots
        const sourceDishes = sourcePlan[swapSource.meal.toLowerCase()]?.[format(swapSource.date, 'yyyy-MM-dd')] || [];
        const targetDishes = (getWeekStart(targetDay) === getWeekStart(swapSource.date) ? sourcePlan : targetPlan)[targetMeal.toLowerCase()]?.[format(targetDay, 'yyyy-MM-dd')] || [];
        ...
        if (getWeekStart(targetDay) === getWeekStart(swapSource.date)) {        // swapping in the same week
            ...
            if (getWeekStart(swapSource.date) === weekStart) {
                setMealPlan(updatedTargetPlan);
            }
            ...
            if (getWeekStart(swapSource.date) === weekStart) {
                setMealPlan(updatedSourcePlan);
            }
            if (getWeekStart(targetDay) === weekStart) {
                setMealPlan(updatedTargetPlan);
            }
